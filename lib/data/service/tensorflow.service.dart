...
import 'package:hw_reader/config/path.constants.dart';
...
class TensorflowService {
  ...
  Future<Uint8List> previewImage(List<Offset> points) async {
    ...
    final image = await picture.toImage(SizeConstants.mnistImageSize, SizeConstants.mnistImageSize);
  ...
  Future recognize(List<Offset> points) async {
    final picture = _pointsToPicture(points);
    Uint8List bytes = await _imageToByteListUint8(picture, SizeConstants.mnistImageSize);
  ...
  Picture _pointsToPicture(List<Offset> points) {
    ...
    final canvas = Canvas(recorder, _canvasCullRect)..scale(SizeConstants.mnistImageSize / SizeConstants.canvasSize);

    canvas.drawRect(Rect.fromLTWH(0, 0, SizeConstants.imageSize, SizeConstants.imageSize), _bgPaint);
